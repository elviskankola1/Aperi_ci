...
var exit_url = "";
var logo_file = "";
var copyright_text = from_email = from_name =  "";
...
if(exit_url == "")
{
    exit_url = "https://www.aperi.ngomadigitech.com/pages"
}
...
window.onload = function () {
    ...
    console.log(token);
    let name  = urlParams.get('user_ref');
    ...
    // var mute_type = localStorage.getItem("mute_type");
    // if (mute_type == 'audio') {
    //     audio_mute = true;
    // }
    // if (mute_type == 'video') {
    ...
    var config = {
        ...
        audio: true,
        data: true,
        audioMuted: audio_mute,
        videoMuted: video_mute,
            ...
            function (success, error) {
                ...
                if (success && success != null) {
                    ...
                    if(room.locked)
                    {
                        ...
                        document.querySelector(".lock_icon").classList.add("fa-unlock");
                        document.querySelector(".lock").innerText = "Unlock";
                    }
                    ...
                    if (isModerator) {
                        ...
                        $("#recording_btn").remove();
                        $("#hangupMeeting").remove();
                        $("#room_lock").remove();
                        $("#mute_all").hide();
                        $("#whiteboard_open").remove();
                        $("#invite_btn").remove();
                        document.getElementById("user-name-div").innerText = language.join_as_part
                        line_icon_class = "cursor-primary";
                    }
                    updateUserList();
                    ...
                    room.addEventListener('stream-subscribed', function (streamEvent) {
                        ...
                        if (active_talker === true) {
                            ...
                            if (stream.ifCanvas() && room.canvasStatus) {
                                canvasStream = stream;
                                streamId = stream.getID();
                                if (presentationStarted == false) {
                                    presentationStarted = true;
                                    if (ATUserList.length > 0) {
                                        addATElement("layout_manager", canvasStream, streamId, "col-md-9", "75%");
                                        managePresentationView();
                                    } else {
                                        addATElement("layout_manager", canvasStream, streamId, "col-md-12", "100%");
                                    }
                                    addMaximizeOption(streamId);
                                    shareStart = true;
                                }
                            }
                        }
                    ...
                    room.addEventListener('active-talkers-updated', function (event) {
                        ...
                        if (ATUserList !== null) {
                            ...
                            var mappedNodes = [...child_nodes]
                                .filter(item => {
                                    if (item.id !== 'con_21' && item.id !== 'con_11' && !item.classList.contains('wb-container')) return item.id;
                                })
                                .map(item => item.id);
                            ...
                            console.log(mappedNodes, difference);
                    ...
                    room.addEventListener('fs-download-result', function (event) {
                        ...
                        switch (evtMsg.messageType) {
                            case 'download-available':
                                ...
                                if (broadCast === false) {
                                    var obj = {
                                        'msg': `${sender} shared a file <br/> File: ${filename} <br/> <a href="#" onclick="file_download(${job_id})">[Download]</span>`,
                    ...
                    room.addEventListener('conference-remaining-duration', function(event){
                        ...
                        // $.confirm({
                        //     title: content ,
                        //     content: "Session closing in next "+timeLeft+" minutes",
                        //     type: 'red',
                        //     animation: 'opacity',
                        ...
                        //             action: function () {
                        //                 room.extendConferenceDuration(function(result){
                        //                     if(result.result== 0)
                        //                     {
                        //                         var extended_minutes = result.extendedTime;
                        //                         toastr.success(`Conference extended to ${extended_minutes} Minutes`);
                        //                     }
                        //                 })
                        //             }
                        //         },
                        ...
                        //             btnClass: 'btn-default w-min-md text-transform-none',
                        //             action: function () {
                        //                 this.close();
                        //             }
                        //         }
                    ...
                    plugin.addEventListener("canvas-stopped", function (event) {
                        var ConfigSpecs = {
                            "maxVideoBW": 0
                        };
                        if (ua.indexOf('safari') != -1) {
                    ...
                    plugin.addEventListener("canvas-started", function (event) {
                        var ConfigSpecs = {
                            "maxVideoBW": 120,
                        };
                        if (ua.indexOf('safari') != -1) {
                        ...
                        if (!isModerator) {
                            ...
                            if (presentationStarted == false) {
                                if (canvasStream == null) {
                                    var st = room.remoteStreams.get(canvas_stream_id);
                                    if (st.stream !== undefined) {
                                        presentationStarted = true;
                                        if (ATUserList.length > 0) {
                                            addATElement("layout_manager", st, canvas_stream_id, "100%", '100%');
                                            managePresentationView();
                                            updateLayoutUponChange(ATUserList, presentationStarted);
                                        } else {
                                            addATElement("layout_manager", st, canvas_stream_id, "100%");
                                            updateLayoutUponChange(ATUserList, presentationStarted);
                                        }
                                        addMaximizeOption(canvas_stream_id);
                                    }
                                } else {
                                    presentationStarted = true;
                                    if (ATUserList.length > 0) {
                                        addATElement("layout_manager", canvasStream, canvas_stream_id, "100%");
                                        managePresentationView();
                                        updateLayoutUponChange(ATUserList, canvas_stream_id);
                                    } else {
                                        addATElement("layout_manager", canvasStream, canvas_stream_id, "100%");
                                        updateLayoutUponChange(ATUserList, presentationStarted);
                                    }
...
function addATElement(elem_id, stream, client_id, div_width, div_heigth = "100%", addCanvasElem = false) {
...
function hangUp() {
    ...
    if(isModerator)
    {
        var chkbox = '<input type="checkbox" id="exit" value="true"/> End meeting on exit';
    }
    $.confirm({
        ...
        buttons: {
            confirm: {
                ...
                action: function () {
                    room.reconnectionAllowed = false;
                    if(isModerator)
                    {
                        var exit_chkbox = document.querySelector("#exit").checked;
                        if(exit_chkbox)
                        {
...
function screenShare() {
    ...
    if (presentationStarted === false) {
        ...
        desktop_shared = false;
        toastr.error("Please stop the Desktop/Application to start Whiteboard.");
        // toastr.error("Presentation is already running");
        // Please stop the Whiteboard to start desktop/application share
    }
    if (streamShare) {
...
function startRoomRec() {
    if (recording_subscription) {
        if (recording_stared) {
            room.stopRecord(function (success) {
                if (success.result == 0) {
                    ...
                    toastr.error("This feature is not available");
                }
            });

        } else {
            room.startRecord(function (success) {
                if (success.result == 0) {
...
function syncLayout(layoutContainer, ATUsersListLength) {
    var { conf_container_inner_height, layoutContainer, finalLayoutWidth } = getLayOutDimensions();
    ...
    if ((presentationStarted && (desktop_shared == false))) {
        ...
        var playerInsidefirstCanvasContainer = document.querySelector('div[id^="con_"]:first-of-type.wb-container');
        ...
        if (playerInsidefirstVideoContainer) {
        ...
        if (playerInsidefirstCanvasContainer) {
            playerInsidefirstCanvasContainer.style.width = `${layoutContainerWidth * 0.75}px`;
            playerInsidefirstCanvasContainer.style.height = `${confoContainerHeight}px`;
        }
        var LMAT = ((presentationStarted && (desktop_shared == false)) && ATUserList.length === 1) ? 2 : gridLayoutTemplate[gridIndex].LMAT;
        document.querySelectorAll('div[id^="con_"]:not(:first-of-type)').forEach(item => {
            item.style.height = `${LMAT > 0 ? parseInt(confoContainerHeight / LMAT) : confoContainerHeight}px`;
    ...
    function getpartToCalculate() {
        ...
        document.querySelectorAll('div[id^="con_"]').forEach((item, index) => {
            ...
            if (((presentationStarted && (desktop_shared == false))) && item.dataset.stream === '21' || item.dataset.stream === '11' && index === 0) {
            ...
            else if ((presentationStarted && (desktop_shared == false)) && item.classList.contains('wb-container') && index === 0) {
                let gridTemplateCols = ``;
                var wbContainer = document.querySelector('div[id^="con_"].wb-container');
                var pWidth = wbContainer.clientWidth;
                ...
                let itemContainer = document.querySelector(`#${item.id}`);
                let itemPlayer = document.querySelector(`#${item.id} .default_vcx_player`);
                if (itemPlayer) {
                    itemPlayer.style.width = `${newWidth}px`;
                    itemPlayer.style.height = `${newHeight}px`;
                }
                if (itemContainer) {
                    itemContainer.style.width = `${newWidth}px`;
...
function lock_unlockRoom() {
    var lockTrigger =  document.querySelector("#room_lock");
    if (room_locked) {
        //unlock room
        room.unlock(function (response) {
            if (response.result == 0) {
                room_locked = false;
                document.querySelector(".lock_icon").classList.remove("fa-unlock");
                document.querySelector(".lock_icon").classList.add("fa-lock");
                document.querySelector(".lock").innerText = "Lock";
                lockTrigger.setAttribute('title', 'Lock Room');
            }
        })
    }
    else {
        // lock room
        room.lock(function (response) {
            if (response.result == 0) {
                room_locked = true;
                document.querySelector(".lock_icon").classList.remove("fa-lock");
                document.querySelector(".lock_icon").classList.add("fa-unlock");
                document.querySelector(".lock").innerText = "Unlock";
                lockTrigger.setAttribute('title', 'Unlock Room');
            }
        })
    }
...
$(document).ready(function () {
    ...
    var whiteBoardTrigger = document.querySelector("#whiteboard_open");
    whiteBoardTrigger.addEventListener("click", function (event) {
        if (shareStart == false) {
            if (presentationStarted) {
                presentationStarted = false;
                destroyWbAndStopStreaming('close');
                whiteBoardTrigger.setAttribute('title', 'Start Whiteboard');
            } else {
                if (!wb) {
                    presentationStarted = true;
                    initAndstartWb();
                    whiteBoardTrigger.setAttribute('title', 'Stop Whiteboard');
                    if (wb && wbSavedState) {
                        wb.setWbData(wbSavedState);
                        wbSavedState = null;
                    }
                }
            }
            ...
            // toastr.error("Screen share is already active.");
            toastr.error("Please stop the Whiteboard to start desktop/application share.");
            //Please stop the Whiteboard to start desktop/application share
        }
    });
    ...
    function handlePublicFileSharing(evt) {
        ...
        var obj = {
            'msg': `You shared a file <br/> File: ${filename}`,
            'timestamp': moment()._d.getTime(),
            'username': "You"
        };
        plotChat(obj);
...
function destroyWbAndStopStreaming(action) {

    if(action === 'close'){
        wbSavedState = wb.getWbData();
    }
    
    if (wb) { wb.stopStreaming(); }
    if (document.querySelector(`#con_wb`)) {
        document.querySelector(`#con_wb`).remove();
    }
    if (document.querySelector(`#con_${room.clientId}`)) {
        document.querySelector(`#con_${room.clientId}`).remove();
    }
    if (ATUserList.length === 0) {
        ATUserList.push({
            streamId: localStream.getID(),
        ...
        addATElement("layout_manager", localStream, room.clientId, "100%", "100%");
        addTitle(localStorage.getItem("userName"), room.clientId, "layout_manager");
    }
    $('.CBicon').removeClass("blink-image");
    wb = null;
    updateLayoutUponChange(ATUserList, presentationStarted);
    vcxSettings.recalculateContainerDimensions();
}

function initAndstartWb() {
    if (room.userList.size === 1) {
        var nodes = document.getElementById('layout_manager').childNodes;
        if (nodes.length > 0) {
            var toRemove = [...nodes].find(item => item.id === `con_wb`);
            if (toRemove) {
                toRemove.remove();
            }
            if (document.querySelector(`#con_${room.clientId}`)) {
    ...
    let wbId = 'wb';
    let whiteBoardContainer = document.createElement('div');
    let wbCanvas = document.createElement('canvas');
    whiteBoardContainer.setAttribute('id', 'whiteboard_div');
    whiteBoardContainer.style = `max-height: 100%;width: 100%;`;
    wbCanvas.setAttribute('id', 'wb');
    whiteBoardContainer.append(wbCanvas);
    addATElement("layout_manager", false, wbId, "100%", "100%", true);
    document.querySelector(`#con_${wbId}`).append(whiteBoardContainer);
    wb = new EnxWb({
        canvasId: 'wb',
        initialWidth: 800,
        initialHeight: 450,
        scheme: 'default',
        language: { exitWb: "Save Whiteboard content befor closing" },
        downloadAndExit: function () {
            console.log('calling from user defined fx');
            if (confirm(wb.language.exitWb)) {
                ...
                if (presentationStarted) {
                    ...
                    wb.stopStreaming();
                    console.log('Destroying streaming..');
                    destroyWbAndStopStreaming('destroy');
                    presentationStarted = false;
                }
            }
            else {
    ...
    $('.CBicon').addClass("blink-image");
    if (document.querySelector('.wb-toolbar-ul')) {
        document.querySelector('.wb-toolbar-ul').parentNode.style.width = '100%';
    }
    vcxSettings.recalculateContainerDimensions();
}
function disconnetUser(element)
{
    var clienName = element.dataset.clientusername;
    var clientId = element.id.split("diconnectUser")[1];
    $.confirm({
        title: "Want to drop <b>"+clienName+"</b> from Conference ? " ,
        content: "",
        animation: 'opacity',
        closeAnimation: 'opacity',
        columnClass: 'col-md-4 offset-md-4 col-sm-6 offset-sm-3 col-xs-10 offset-xs-1',
            ...
            confirm: {
                ...
                action: function () {
                    room.dropUser([clientId],function(res){
                        if(res.result == 0)
                        {
                            toastr.success("User Disconnected");
                        }
                    })
                }
            },
...
function resizeWhiteboardCanvas() {
    const outerCanvasContainer = $('#whiteboard_div')[0];
    if (wb && outerCanvasContainer) {
...
$(window).resize(resizeWhiteboardCanvas);